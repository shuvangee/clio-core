...
naturally with jarvis_iowarp.wrp_cte configured at 513.0.
...
context-transfer-engine; wrp_runtime loads it at daemon start.
"""
...
class WrpCompress(Service):
    ...
    deploy_mode='container': runs `chimaera compose` inside the deploy
                             container (shares wrp_runtime's image).
    """
    ...
    def _configure(self, **kwargs):
        ...
        self.compose_config_path = os.path.join(
            self.shared_dir, 'compress_compose.yaml')
        self.log("Configuring transparent compression service (wrp_compress)...")
        ...
        with open(self.compose_config_path, 'w') as f:
            f.write('# wrp_compress chimaera-compose configuration\n\n')
        ...
        self.log(f"wrp_compress: compose written to {self.compose_config_path} "
    ...
    def start(self):
        self.log("Starting wrp_compress via chimaera compose...")
        ...
        self.log("wrp_compress started successfully")
        return True
