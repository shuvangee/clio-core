...
CTP_INDIRECTLY_CALLABLE static CTP_GPU_FUN ctp::ipc::FullPtr<chi::Task> AllocTaskImpl(
  ...
  switch (method) {
    case Method::kGpuSubmit: {
      auto _tp = CLIO_IPC->NewTaskBase<GpuSubmitTask>(0); return _tp.template Cast<chi::Task>();
    }
    case Method::kSubtaskTest: {
      auto _tp = CLIO_IPC->NewTaskBase<SubtaskTestTask>(0); return _tp.template Cast<chi::Task>();
    }
