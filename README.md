...
int main() {
  ...
  // 2. Open (or create) a tag — a named container for blobs.  Storage
  //    targets are configured declaratively via the runtime's compose
  //    YAML, so the example doesn't need to RegisterTarget by hand.
  clio::cte::core::Tag tag("my_tag");

  // 3. Store blob data.
  std::vector<char> data(4096, 'A');
  tag.PutBlob("my_blob", data.data(), data.size());

  // 4. Retrieve blob data.
  std::vector<char> read_buf(data.size());
  tag.GetBlob("my_blob", read_buf.data(), read_buf.size());
